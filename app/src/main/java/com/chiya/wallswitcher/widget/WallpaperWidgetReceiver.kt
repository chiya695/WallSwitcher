    ...
    private fun handleSwitchWallpaper(context: Context) {
        CoroutineScope(Dispatchers.Default).launch {
            try {
                ...
                // 获取下一张壁纸
                val nextWallpaper = if (settings.switchOrder == SwitchOrder.SEQUENTIAL.ordinal) {
                    // 顺序切换
                ...
                LogUtils.log("切换壁纸时出错: ${e.message}")
                e.printStackTrace()
            }
        }
    }
